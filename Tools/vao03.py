...
COMPLEX_INTERACTION_CAPABILITIES = {
    STATEFUL_INTERACTION, CONDITIONAL_ROUTING, COMPOSITE_ACTUATION,
    TIMED_PROCESS, ACTUATOR_TRANSFER, CAPTURE_EVENT_ALIGNMENT,
}
PLAYABLE_CAPABILITIES = {
    INTERACTION, PLAYABLE_INTERACTION, SAMPLED_INSTRUMENT_PLAYBACK,
    SAMPLE_LOOPING, TUNING_MAP, MULTI_PERSPECTIVE_SAMPLING,
    RECORDED_RELEASE, SOURCE_SAMPLER_SEMANTICS, *COMPLEX_INTERACTION_CAPABILITIES,
}
...
def validate_playable(
    ...
    profile_records = [
        record for record in manifest.get("profiles", []) + manifest.get("materializableProfiles", [])
        if isinstance(record, dict)
    ]
    playable_profiles = [record for record in profile_records if record.get("id") == PLAYABLE_PROFILE]
    profile_ids = {record.get("id") for record in profile_records}
    declares_profile = bool(playable_profiles)
    capabilities = {
        capability for record in playable_profiles
    ...
    if declares_profile:
        ...
        interactions = [record for record in entities.values() if record.get("kind") == "interaction"]
        if not interactions:
        ...
        for interaction in interactions:
            ...
            for local_name in ("interactionType", "controlProtocol", "controlDomain", "timingPolicy"):
                if "https://w3id.org/modavis/vao/ontology#" + local_name not in properties:
            ...
            if not any(
                ...
                errors.append(f"Playable interaction {interaction.get('id')!r} has no active activates/modulates relation.")
    ...
    sample_capabilities = capabilities & {
        SAMPLED_INSTRUMENT_PLAYBACK, SAMPLE_LOOPING, TUNING_MAP,
        MULTI_PERSPECTIVE_SAMPLING, RECORDED_RELEASE, SOURCE_SAMPLER_SEMANTICS,
    }
    ...
    acoustic_pose_ids = {
        record.get("id") for record in acoustics.get("poses", [])
    ...
    def evidence(record: dict[str, Any], label: str) -> None:
        ...
        if (record.get("status") == "inferred" or record.get("source") == "algorithmic") and generated_by is None:
            errors.append(f"{label} {identifier!r} is inferred or algorithmic but lacks generatedById.")
        if generated_by is not None and generated_by not in known_activity_ids:
            errors.append(f"{label} {identifier!r} has unresolved generatedById {generated_by!r}.")
        if record.get("source") == "algorithmic" and record.get("status") == "asserted":
            errors.append(f"{label} {identifier!r} cannot describe algorithmic output as asserted source metadata.")

    for region in regions.values():
        evidence(region, "Signal region")
        realization = realizations.get(region.get("realizationId"))
        if realization is None:
            errors.append(f"Signal region {region['id']!r} has unresolved realizationId.")
            continue
        technical = realization.get("technicalMetadata", {})
        if technical.get("kind") != "audio":
            errors.append(f"Signal region {region['id']!r} must reference an audio realization.")
        start = region.get("startFrameInclusive")
        end = region.get("endFrameExclusive")
        if isinstance(start, int) and isinstance(end, int) and end <= start:
            errors.append(f"Signal region {region['id']!r} has an empty or reversed half-open frame range.")
        frame_count = technical.get("frameCount")
        if isinstance(end, int) and isinstance(frame_count, int) and end > frame_count:
            errors.append(f"Signal region {region['id']!r} exceeds the realization frameCount.")
    ...
    for loop in loops.values():
        ...
        for region_id in loop.get("regionIds", []):
            if region_id not in regions:
                errors.append(f"Loop-point set {loop['id']!r} has unresolved region {region_id!r}.")
            else:
                resolved.append(regions[region_id])
        if any(region.get("role") != "sustain-loop" for region in resolved):
            errors.append(f"Every region in loop-point set {loop['id']!r} must have role sustain-loop.")
        if len({region.get("realizationId") for region in resolved}) > 1:
            errors.append(f"Loop-point set {loop['id']!r} spans more than one realization.")
        crossfade = loop.get("crossfadeFrames")
        lengths = [region.get("endFrameExclusive", 0) - region.get("startFrameInclusive", 0) for region in resolved]
        if isinstance(crossfade, int) and lengths and crossfade > min(lengths):
            errors.append(f"Loop-point set {loop['id']!r} has a crossfade longer than a loop region.")
        reviewed_by = loop.get("reviewedById")
        if reviewed_by is not None and reviewed_by not in known_activity_ids:
            errors.append(f"Loop-point set {loop['id']!r} has unresolved reviewedById {reviewed_by!r}.")

    for tuning in tunings.values():
        evidence(tuning, "Tuning map")
        keys = [entry.get("key") for entry in tuning.get("entries", []) if isinstance(entry, dict)]
    ...
    for perspective in perspectives.values():
        ...
        for realization_id in perspective.get("realizationIds", []):
            ...
            if realization is None:
                errors.append(f"Perspective group {perspective['id']!r} has unresolved realization {realization_id!r}.")
                continue
            technical = realization.get("technicalMetadata", {})
            if technical.get("kind") != "audio":
                errors.append(f"Perspective group {perspective['id']!r} must reference audio realizations.")
            channel_count = technical.get("channelCount")
            if isinstance(channel_count, int) and any(index >= channel_count for index in perspective.get("channelIndices", [])):
                errors.append(f"Perspective group {perspective['id']!r} contains a channel outside realization {realization_id!r}.")
        for pose_id in perspective.get("poseIds", []):
            if pose_id not in acoustic_pose_ids:
                errors.append(f"Perspective group {perspective['id']!r} has unresolved acoustic pose {pose_id!r}.")
        if perspective.get("geometryStatus") == "position-registered" and not perspective.get("poseIds"):
            errors.append(f"Position-registered perspective group {perspective['id']!r} requires poseIds.")
        if perspective.get("geometryStatus") == "position-registered" and SPATIAL_PROFILE not in profile_ids:
            errors.append(f"Position-registered perspective group {perspective['id']!r} requires the Spatial 0.3 profile.")
    ...
    for variant in variants.values():
        ...
        realization = realizations.get(realization_id)
        if realization is None:
            errors.append(f"Sample variant {variant['id']!r} has unresolved realizationId.")
            continue
        if realization.get("technicalMetadata", {}).get("kind") != "audio":
            errors.append(f"Sample variant {variant['id']!r} must reference an audio realization.")
        for region_id in variant.get("signalRegionIds", []):
            region = regions.get(region_id)
            if region is None:
                errors.append(f"Sample variant {variant['id']!r} has unresolved signal region {region_id!r}.")
            elif region.get("realizationId") != realization_id:
                errors.append(f"Sample variant {variant['id']!r} references a region from another realization.")
            elif not usable(region.get("status")):
                errors.append(f"Sample variant {variant['id']!r} uses a rejected, superseded, or unreviewed inferred region {region_id!r}.")
        for loop_id in variant.get("loopPointSetIds", []):
            loop = loops.get(loop_id)
            if loop is None:
                errors.append(f"Sample variant {variant['id']!r} has unresolved loop-point set {loop_id!r}.")
                continue
            realization_ids = {regions[region_id].get("realizationId") for region_id in loop.get("regionIds", []) if region_id in regions}
            if realization_ids != {realization_id}:
                errors.append(f"Sample variant {variant['id']!r} references a loop from another realization.")
            if not usable(loop.get("status")):
                errors.append(f"Sample variant {variant['id']!r} uses a rejected, superseded, or unreviewed inferred loop {loop_id!r}.")
        ...
        if perspective_id is not None:
            ...
            if perspective is None:
                errors.append(f"Sample variant {variant['id']!r} has unresolved perspectiveGroupId.")
            elif realization_id not in perspective.get("realizationIds", []):
                errors.append(f"Sample variant {variant['id']!r} is not a member of its perspective group.")
            elif not usable(perspective.get("status")):
                errors.append(f"Sample variant {variant['id']!r} uses a rejected, superseded, or unreviewed inferred perspective group.")
    ...
    for mapping in mappings.values():
        ...
        if key_range.get("minimum", 0) > key_range.get("maximum", 0):
            errors.append(f"Sample mapping {mapping['id']!r} has a reversed MIDI key range.")
        if velocity_range.get("minimum", 0) > velocity_range.get("maximum", 0):
            errors.append(f"Sample mapping {mapping['id']!r} has a reversed velocity range.")
        for key in ("instrumentEntityId", "componentEntityId", "rankEntityId"):
            ...
            if entity_id is not None and entity_id not in entities:
                errors.append(f"Sample mapping {mapping['id']!r} has unresolved entity {entity_id!r}.")
        instrument = entities.get(mapping.get("instrumentEntityId"))
        if instrument is not None and instrument.get("kind") != "instrument":
            errors.append(f"Sample mapping {mapping['id']!r} instrumentEntityId does not identify an instrument entity.")
        tuning_id = mapping.get("tuningMapId")
        if tuning_id is not None and tuning_id not in tunings:
            errors.append(f"Sample mapping {mapping['id']!r} has unresolved tuningMapId.")
        elif tuning_id is not None and not usable(tunings[tuning_id].get("status")):
            errors.append(f"Sample mapping {mapping['id']!r} uses a rejected, superseded, or unreviewed inferred tuning map.")
        resolved_variants = []
        for variant_id in mapping.get("variantIds", []):
            if variant_id not in variants:
                errors.append(f"Sample mapping {mapping['id']!r} has unresolved variant {variant_id!r}.")
            else:
                resolved_variants.append(variants[variant_id])
        if not any(variant.get("trigger") == "note-on" for variant in resolved_variants):
            errors.append(f"Sample mapping {mapping['id']!r} requires at least one note-on variant.")
        if any(not usable(variant.get("status")) for variant in resolved_variants):
            errors.append(f"Sample mapping {mapping['id']!r} references rejected, superseded, or unreviewed inferred variants.")
        if mapping.get("noteOffPolicy") == "recorded-release" and not any(
            variant.get("trigger") == "note-off" and variant.get("signalRole") == "release"
            for variant in resolved_variants
        ):
            errors.append(f"Sample mapping {mapping['id']!r} claims recorded-release without a note-off release variant.")
    ...
    mapped_variants = [variants[identifier] for identifier in mapped_variant_ids if identifier in variants]
    if SAMPLE_LOOPING in capabilities and not any(variant.get("loopPointSetIds") for variant in mapped_variants):
        errors.append("The sample-looping capability requires a mapped sample variant bound to a loop-point set.")
    ...
    mapped_perspectives = {variant.get("perspectiveGroupId") for variant in mapped_variants if variant.get("perspectiveGroupId") in perspectives}
    ...
    if RECORDED_RELEASE in capabilities and not any(mapping.get("noteOffPolicy") == "recorded-release" for mapping in mappings.values()):
    ...
    if SOURCE_SAMPLER_SEMANTICS in capabilities:
        source_backed = any(mapping.get("source") == "native-definition" for mapping in mappings.values()) and any(
            variant.get("source") in {"native-definition", "embedded-metadata"} for variant in variants.values()
        )
...
def validate_complex_interaction(
    ...
    capabilities = {capability for record in profile_records for capability in record.get("requiredCapabilities", [])}
    ...
    if CAPTURE_EVENT_ALIGNMENT in capabilities and not isinstance(capture_documentation, dict):
    ...
    if not isinstance(interaction_model, dict) and not isinstance(capture_documentation, dict):
        return
    ...
    def evidence(record: dict[str, Any], label: str) -> None:
        ...
        if (record.get("status") == "inferred" or record.get("source") == "algorithmic") and generated_by is None:
            errors.append(f"{label} {record.get('id')!r} is inferred or algorithmic but lacks generatedById.")
        if generated_by is not None and generated_by not in known_activity_ids:
            errors.append(f"{label} {record.get('id')!r} has unresolved generatedById {generated_by!r}.")
        if reviewed_by is not None and reviewed_by not in known_activity_ids:
            errors.append(f"{label} {record.get('id')!r} has unresolved reviewedById {reviewed_by!r}.")
        if record.get("source") == "algorithmic" and record.get("status") == "asserted":
            errors.append(f"{label} {record.get('id')!r} cannot describe algorithmic output as asserted source metadata.")

    def check_condition(condition: Any, owner: str) -> None:
        if isinstance(condition, dict) and condition.get("stateVariableId") not in states:
            errors.append(f"{owner} has unresolved stateVariableId {condition.get('stateVariableId')!r}.")
    ...
    def check_action(action: Any, owner: str) -> None:
        ...
        targets = {
            "set-state": states, "toggle-state": states, "increment-state": states,
            "emit-event": events, "start-process": processes, "stop-process": processes,
            "route-event": routes, "select-render-binding": renders,
        }.get(operation, {})
        if target not in targets:
            errors.append(f"{owner} action {operation!r} has unresolved or incompatible targetId {target!r}.")
        ...
        if operation == "increment-state" and not isinstance(action.get("value"), (int, float)):
    ...
    for control in controls.values():
        ...
        if entity_id is not None and entity_id not in entities:
            errors.append(f"Interaction control {control['id']!r} has unresolved entityId.")
        minimum, maximum = control.get("minimumValue"), control.get("maximumValue")
        if isinstance(minimum, (int, float)) and isinstance(maximum, (int, float)) and minimum > maximum:
            errors.append(f"Interaction control {control['id']!r} has reversed value bounds.")
        if control.get("controlBehavior") == "stepped" and not isinstance(control.get("stepCount"), int):
            errors.append(f"Stepped interaction control {control['id']!r} requires stepCount.")
        if control.get("valueType") == "enumerated" and not control.get("allowedValues"):
            errors.append(f"Enumerated interaction control {control['id']!r} requires allowedValues.")
    ...
    for binding in bindings.values():
        ...
        if binding.get("controlId") not in controls:
            errors.append(f"Protocol binding {binding['id']!r} has unresolved controlId.")
        if binding.get("eventTypeId") not in events:
            errors.append(f"Protocol binding {binding['id']!r} has unresolved eventTypeId.")
        if binding.get("protocol") in {"MIDI-1.0", "MIDI-2.0"}:
            ...
            if channel_base not in {0, 1} or data_base not in {0, 1}:
                errors.append(f"MIDI protocol binding {binding['id']!r} requires explicit channelNumberingBase and dataNumberingBase.")
            if not isinstance(channel, int) or channel_base not in {0, 1} or not channel_base <= channel <= channel_base + 15:
                errors.append(f"MIDI protocol binding {binding['id']!r} has a channel outside its declared numbering base.")
            if binding.get("messageType") not in {"pitch-bend", "channel-pressure", "system-exclusive"}:
                if not isinstance(number, int) or data_base not in {0, 1} or not data_base <= number <= data_base + 127:
                    errors.append(f"MIDI protocol binding {binding['id']!r} has a number outside its declared numbering base.")
        elif binding.get("messageType") == "addressed-value" and not binding.get("address"):
            errors.append(f"Addressed protocol binding {binding['id']!r} requires address.")

    for state in states.values():
        evidence(state, "State variable")
        subject = state.get("subjectEntityId")
        if subject is not None and subject not in entities:
            errors.append(f"State variable {state['id']!r} has unresolved subjectEntityId.")
        ...
        if state.get("valueType") == "enumerated" and state.get("defaultValue") not in state.get("allowedValues", []):
            errors.append(f"Enumerated state variable {state['id']!r} has a default outside allowedValues.")

    for transition in transitions.values():
        evidence(transition, "Interaction transition")
        if transition.get("eventTypeId") not in events:
            errors.append(f"Interaction transition {transition['id']!r} has unresolved eventTypeId.")
        control_id = transition.get("controlId")
        if control_id is not None and control_id not in controls:
            errors.append(f"Interaction transition {transition['id']!r} has unresolved controlId.")
    ...
    for route in routes.values():
        ...
        if source_id not in entities or target_id not in entities:
            errors.append(f"Routing rule {route['id']!r} has an unresolved source or target entity.")
        ...
        if source_control is not None and source_control not in controls:
            errors.append(f"Routing rule {route['id']!r} has unresolved sourceControlId.")
        ...
        if kind == "transpose" and not isinstance(transform.get("semitoneOffset"), int):
            errors.append(f"Routing rule {route['id']!r} transpose transform requires semitoneOffset.")
        if kind == "transpose" and isinstance(transform.get("semitoneOffset"), int):
            offset = transform["semitoneOffset"]
            if key_range.get("minimum", 0) + offset < 0 or key_range.get("maximum", 127) + offset > 127:
                errors.append(f"Routing rule {route['id']!r} transposes outside the MIDI key domain.")
        if kind == "fixed" and not transform.get("fixedOutputKeys"):
            errors.append(f"Routing rule {route['id']!r} fixed transform requires fixedOutputKeys.")
        if kind == "table":
            table_inputs = [entry.get("inputKey") for entry in transform.get("entries", []) if isinstance(entry, dict)]
            if not table_inputs or len(table_inputs) != len(set(table_inputs)):
                errors.append(f"Routing rule {route['id']!r} table transform needs unique entries.")
    ...
    for timing in timings.values():
        ...
        values = [timing.get(key) for key in ("minimum", "typical", "maximum") if isinstance(timing.get(key), (int, float))]
        if values != sorted(values):
            errors.append(f"Timing constraint {timing['id']!r} has inconsistent minimum/typical/maximum ordering.")
        for target in timing.get("appliesToIds", []):
            if target not in controls and target not in routes and target not in processes and target not in entities:
                errors.append(f"Timing constraint {timing['id']!r} has unresolved appliesToId {target!r}.")

    process_edges: dict[str, list[str]] = {}
    for process in processes.values():
        evidence(process, "Process model")
        children = process.get("childProcessIds", [])
        process_edges[process["id"]] = [child for child in children if child in processes]
        for child in children:
            if child not in processes:
                errors.append(f"Process model {process['id']!r} has unresolved child process {child!r}.")
        for timing_id in process.get("timingConstraintIds", []):
            if timing_id not in timings:
                errors.append(f"Process model {process['id']!r} has unresolved timing constraint {timing_id!r}.")
        ...
        if kind in {"repeating", "stochastic"} and termination == "completed":
            errors.append(f"Process model {process['id']!r} is potentially unbounded and needs an explicit termination mechanism.")
        if termination == "maximum-iterations" and not isinstance(process.get("maximumIterations"), int):
            errors.append(f"Process model {process['id']!r} requires maximumIterations.")
        if termination == "duration-bound" and process.get("durationConstraintId") not in timings:
            errors.append(f"Process model {process['id']!r} requires a resolved durationConstraintId.")
        if termination in {"on-control-release", "external-cancel"} and process.get("cancellationControlId") not in controls:
            errors.append(f"Process model {process['id']!r} requires a resolved cancellationControlId.")
        if kind == "compound" and len(process.get("actions", [])) + len(children) < 2:
            errors.append(f"Compound process model {process['id']!r} requires at least two actions or child processes.")
    find_cycles(process_edges, "Interaction process graph")

    for transfer in transfers.values():
        evidence(transfer, "Transfer function")
        inputs = [point.get("input") for point in transfer.get("points", []) if isinstance(point, dict)]
        if inputs != sorted(inputs) or len(inputs) != len(set(inputs)):
            errors.append(f"Transfer function {transfer['id']!r} points must have unique ascending inputs.")
        for target in transfer.get("appliesToIds", []):
            if target not in controls and target not in states and target not in routes and target not in playable_records["sampleMappings"]:
                errors.append(f"Transfer function {transfer['id']!r} has unresolved appliesToId {target!r}.")
    ...
    for render in renders.values():
        ...
        if event_id is not None and event_id not in events:
            errors.append(f"Render binding {render['id']!r} has unresolved eventTypeId.")
        if process_id is not None and process_id not in processes:
            errors.append(f"Render binding {render['id']!r} has unresolved processModelId.")
        ...
        for mapping_id in render.get("sampleMappingIds", []):
            if mapping_id not in playable_records["sampleMappings"]:
                errors.append(f"Render binding {render['id']!r} has unresolved sample mapping {mapping_id!r}.")
        for variant_id in render.get("sampleVariantIds", []):
            if variant_id not in playable_records["sampleVariants"]:
                errors.append(f"Render binding {render['id']!r} has unresolved sample variant {variant_id!r}.")

    if STATEFUL_INTERACTION in capabilities and not (states and transitions):
        errors.append("The stateful-interaction capability requires state variables and transitions.")
    if CONDITIONAL_ROUTING in capabilities and not any(route.get("conditions") for route in routes.values()):
        errors.append("The conditional-routing capability requires a routing rule with state conditions.")
    if COMPOSITE_ACTUATION in capabilities and not any(
        process.get("processKind") == "compound" and process.get("status") in usable_statuses
        for process in processes.values()
    ):
        errors.append("The composite-actuation capability requires a usable compound process model.")
    if TIMED_PROCESS in capabilities and not any(process.get("timingConstraintIds") for process in processes.values()):
        errors.append("The timed-process capability requires a process bound to timing constraints.")
    if ACTUATOR_TRANSFER in capabilities and not any(transfer.get("status") in usable_statuses for transfer in transfers.values()):
        errors.append("The actuator-transfer capability requires a usable transfer function.")
    ...
    for capture_state in capture_states.values():
        ...
        if instrument is None or instrument.get("kind") != "instrument":
            errors.append(f"Capture state {capture_state['id']!r} does not resolve an instrument entity.")
        assignment_ids = [item.get("stateVariableId") for item in capture_state.get("stateAssignments", []) if isinstance(item, dict)]
        if len(assignment_ids) != len(set(assignment_ids)):
            errors.append(f"Capture state {capture_state['id']!r} assigns a state variable more than once.")
        for state_id in assignment_ids:
            if state_id not in states:
                errors.append(f"Capture state {capture_state['id']!r} has unresolved state variable {state_id!r}.")
    ...
    for alignment in alignments.values():
        ...
        if audio is None or audio.get("technicalMetadata", {}).get("kind") != "audio":
            errors.append(f"Event alignment {alignment['id']!r} must reference an audio realization.")
        if event_log is None:
            errors.append(f"Event alignment {alignment['id']!r} has unresolved event-log realization.")
        frame_count = audio.get("technicalMetadata", {}).get("frameCount") if audio else None
        for event_mapping in alignment.get("eventMappings", []):
            ...
            if event_id is not None and event_id not in events:
                errors.append(f"Event alignment {alignment['id']!r} has unresolved eventTypeId {event_id!r}.")
            start, end = event_mapping.get("startFrame"), event_mapping.get("endFrameExclusive")
            if isinstance(end, int) and isinstance(start, int) and end <= start:
                errors.append(f"Event alignment {alignment['id']!r} has an empty event frame range.")
            if isinstance(end, int) and isinstance(frame_count, int) and end > frame_count:
                errors.append(f"Event alignment {alignment['id']!r} exceeds audio frameCount.")
    ...
    for take_set in take_sets.values():
        ...
        for realization_id in take_set.get("realizationIds", []):
            if realization_id not in realizations:
                errors.append(f"Take set {take_set['id']!r} has unresolved realization {realization_id!r}.")
        if take_set.get("setRole") == "round-robin" and take_set.get("selectionStatus") != "accepted":
            errors.append(f"Round-robin take set {take_set['id']!r} must be explicitly accepted.")

    derivation_edges: dict[str, list[str]] = {}
    for derivation in derivations.values():
        evidence(derivation, "Derivation map")
        source_id, derived_id = derivation.get("sourceRealizationId"), derivation.get("derivedRealizationId")
        source_realization, derived_realization = realizations.get(source_id), realizations.get(derived_id)
        if source_realization is None or derived_realization is None:
            errors.append(f"Derivation map {derivation['id']!r} has an unresolved source or derived realization.")
        elif source_id == derived_id:
            errors.append(f"Derivation map {derivation['id']!r} cannot derive a realization from itself.")
        else:
            derivation_edges.setdefault(source_id, []).append(derived_id)
        frame_range = derivation.get("sourceFrameRange", {})
        start, end = frame_range.get("startFrameInclusive"), frame_range.get("endFrameExclusive")
        if isinstance(start, int) and isinstance(end, int) and end <= start:
            errors.append(f"Derivation map {derivation['id']!r} has an empty source frame range.")
        frame_count = source_realization.get("technicalMetadata", {}).get("frameCount") if source_realization else None
        if isinstance(end, int) and isinstance(frame_count, int) and end > frame_count:
            errors.append(f"Derivation map {derivation['id']!r} exceeds source frameCount.")
        channel_count = source_realization.get("technicalMetadata", {}).get("channelCount") if source_realization else None
        if isinstance(channel_count, int) and any(index >= channel_count for index in derivation.get("channelMap", [])):
            errors.append(f"Derivation map {derivation['id']!r} contains an out-of-range source channel.")
        for operation in derivation.get("operations", []):
            activity_id = operation.get("activityId") if isinstance(operation, dict) else None
            if activity_id is not None and activity_id not in known_activity_ids:
                errors.append(f"Derivation map {derivation['id']!r} has unresolved operation activityId {activity_id!r}.")
    ...
    if CAPTURE_EVENT_ALIGNMENT in capabilities and not any(
        ...
        errors.append("The capture-event-alignment capability requires a usable event alignment.")

    for mapping in playable_records["sampleMappings"].values():
        if mapping.get("velocitySensitivity") == "transfer-function" and not any(
            mapping["id"] in transfer.get("appliesToIds", []) for transfer in transfers.values()
        ):
            errors.append(f"Sample mapping {mapping['id']!r} declares transfer-function velocity sensitivity without an applicable transfer function.")
...
def validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    playable = manifest.get("playable") if isinstance(manifest.get("playable"), dict) else {}
    playable_records = {
        key: records_by_id(playable.get(key), f"playable.{key}", errors, global_ids)
        for key in (
            "signalRegions", "loopPointSets", "tuningMaps", "perspectiveGroups",
            "sampleVariants", "sampleMappings",
        )
    }
    interaction_model = manifest.get("interactionModel") if isinstance(manifest.get("interactionModel"), dict) else {}
    interaction_records = {
        key: records_by_id(interaction_model.get(key), f"interactionModel.{key}", errors, global_ids)
        for key in (
    ...
    capture_documentation = manifest.get("captureDocumentation") if isinstance(manifest.get("captureDocumentation"), dict) else {}
    capture_records = {
        key: records_by_id(capture_documentation.get(key), f"captureDocumentation.{key}", errors, global_ids)
    ...
    validate_playable(
        manifest, entities, realizations, paradata, analyses, relations,
        playable_records, errors,
    )
    validate_complex_interaction(
        manifest, entities, realizations, paradata, analyses, relations,
        interaction_records, capture_records, playable_records, errors,
    )
...
def validate_carrier_parts(
    ...
    for mapping in carrier.get("embeddedRealizations", []):
        ...
        normalized_path = normalized_carrier_path(path) if isinstance(path, str) else f"<invalid:{path!r}>"
        ...
        with zipfile.ZipFile(path, "r", allowZip64=True) as archive:
            ...
            for info in infos:
                ...
                if normalized_name in names:
                    errors.append(f"Duplicate archive path after NFC normalization: {name!r}.")
