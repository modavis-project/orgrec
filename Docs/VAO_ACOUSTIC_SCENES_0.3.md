# VAO 0.3.3 visual-acoustic scene implementation guide
...
The current Blender add-on can remain a 0.2.2 importer while VAO 0.3.3 standardizes the information it will need. A future add-on update should add 0.3.3 dispatch, choose a `runtime-visual` glTF realization, evaluate the frame graph, create source/receiver objects keyed by stable entity IDs, and expose measurement/response links. None of those application behaviors are required to validate or preserve a VAO.
