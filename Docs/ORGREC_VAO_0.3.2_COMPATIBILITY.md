...
OrgRec supports VAO 0.3.2 as an instrument-neutral, read-only inspection and workspace-import format. This is separate from OrgRec's editable VAO 0.2.2 capture-project round trip. A visual-acoustic scene whose primary entity is a room is not converted into a pipe-organ recording project and is not silently rewritten to 0.2.2. Playable 0.3.2 manifests receive typed read-only access to mappings, variants, signal/loop regions, tuning, perspectives, and their exact embedded audio resources.
...
- playable sample mappings and note-on/note-off variants;
- half-open signal/loop regions, source/status state, and audio frame bounds;
- tuning maps, microphone-perspective groups, round-robin identity, and recorded-release policy;
- exact playable audio realization metadata, fixity, and embedded path;
